...
class WebsiteController extends Controller
{
    ...
    public function showCertificate(Request $request)
    {
        try {
            ...
            if ($certificate_record) {
                ...
                if (!$certificate) {
                    Toastr::error(trans('Certificado no encontrado'));
                    return back();
                }
                ...
                return Redirect::back()->withErrors(['msg', 'Número de certificado invalido']);
            }
    ...
    public function certificateCheck($certificate_number, Request $request)
    {
        try {
            ...
            if (!$certificate) {
                Toastr::error(trans('No puedes descargar este certificado'));
                return back();
            }
    ...
    public function certificateDownload($certificate_number, Request $request)
    {
        try {
            ...
            if (!$certificate) {
                Toastr::error(trans('No puedes descargar este certificado ahora'));
                return back();
            }
        ...
        try {
            ...
            if (isModuleActive('BundleSubscription')) {
                if (isBundleExpire($course_id)) {
                    Toastr::error('La validez de tu paquete expiró', 'Acceso denegadi');
                    return redirect()->back();
                }
            }
            ...
            if ($lesson->is_lock == 1) {
                if (!Auth::check()) {
                    Toastr::error('No tienes acceso a este curso', 'Error');
                    return redirect()->back();
                } else {
            ...
            if ($course->drip != 0) {
                ...
                if (!$lessonShow) {
                    Toastr::error('Lección actualmente no disponible', 'Error');
                    return redirect()->back();
                }
            }
            ...
            if (!$isAdmin) {
                if ($course->complete_order == 1) {
                    if (!Auth::check()) {
                        Toastr::error('Debes iniciar sesión para continuar', 'Error');
                        return redirect()->back();
                    }
                    ...
                    if ($previous) {
                        ...
                        if (!$isComplete || $isComplete->status != 1) {
                            Toastr::error(trans('frontend.Debes completar antes la lección anterior'), trans('Error'));
                            return redirect()->back();
                        }
                    }
                }
        ...
        try {
            if (!hasTable('newsletter_settings')) {
                ...
                if (empty($check)) {
                    ...
                    Toastr::error('Ya estás suscrito', 'Error');
                }
            } else {
                ...
                if ($newsletterSetting->home_service == "Local") {
                    ...
                    if (saasEnv('MailChimp_Status') == "true") {
                        ...
                        if ($MailChimp->success()) {
                            ...
                            Toastr::error(json_decode($MailChimp->getLastResponse()['body'], TRUE)['title'] ?? 'Algo ha salido mal', trans('common.Error'));
                            return Redirect::back();
                        }
                    }
                    ...
                    if (saasEnv('GET_RESPONSE_STATUS') == "true") {
                        ...
                        if (empty($callback)) {
                            ...
                            Toastr::error($callback->message ?? 'Algo salió mal', trans('Error'));
                            return Redirect::back();
                        }
                    }
                    ...
                    if (saasEnv('ACELLE_STATUS') == "true") {
                        ...
                        if ($response) {
                            ...
                            Toastr::error('Algo salió mal', trans('Error'));
                            return Redirect::back();
                        }
                    }
                }
    ...
    public function addToCart($id)
    {
        try {
            ...
            if (!$course) {
                Toastr::error('Curso no encontrado', 'Error');
                return redirect()->back();
            }
    ...
    public function buyNow($id)
    {
        try {
            if (!Auth::check()) {
                Toastr::error('Debe iniciar sesión', 'Error');
                return \redirect()->route('login');
            }
            $course = Course::find($id);
            if (!$course) {
            ...
            if ($course->isLoginUserEnrolled) {
                Toastr::error(trans('frontend.Ya estas matriculado en este curso'), 'error');
                return redirect()->back();
            }
            if (isModuleActive('Org')) {
                $type = $course->required_type;
            ...
            if (!empty($qusAns)) {
                ...
                if (empty($test)) {
                    ...
                    if ($question_review == 1) {
                        ...
                        return response()->json(['error' => 'Ya enviado'], 500);
                    }
...
//                        $totalScore +=$ans->
    ...
    public function frontPage($slug)
    {
        try {
            ...
            if (!$page) {
                Toastr::error(trans('Error'), trans('Error'));
                return redirect()->back();
            }

            if ($page->status != 1) {
                Toastr::error('Lo sentimos, esta página no está activa', trans('Error'));
                return redirect()->back();
            }
    ...
    public function enrollOrCart($id)
    {
        ...
        try {
            ...
            if (Auth::check() && ($user->role_id != 1)) {
                ...
                if (!$course) {
                    $output['result'] = 'Error';
                    $output['message'] = 'Curso no encontrado';
            ...
            $output['result'] = 'Error';
            $output['message'] = 'Operación fallida';
        }
    ...
    public function subscriptionCourses()
    {
        ...
        if (isModuleActive('Subscription')) {
            if (!isSubscribe()) {
                Toastr::error('Debes suscribirte primero', 'Error');
                return redirect()->back();
            }
        }
        try {
        ...
        if (!empty($s_plan) && !empty($price)) {
            if (Auth::check()) {
                if (Auth::user()->role_id == 3) {
                    ...
                    if (!$addCart) {
                        Toastr::error('Solicitud invalida', 'Error');
                        return \redirect()->route('courseSubscription');
                    }
                } else {
                    Toastr::error('Debe iniciar sesión como estudiante', 'Error');
                    return \redirect()->route('courseSubscription');
                }
    ...
    public function continueCourse($slug)
    {
        try {
            ...
            if (!$course) {
                Toastr::error(trans('Operación fallida'), trans('Error'));
                return redirect()->route('courseDetailsView', $slug);
            }
            $isEnrolled = $course->isLoginUserEnrolled;

            if ($isEnrolled) {
                $lesson = LessonComplete::where('course_id', $course->id)->where('user_id', $user->id)->has('lesson')->orderBy('updated_at', 'asc')->first();
                if (empty($lesson)) {
                    $chapter = Chapter::where('course_id', $course->id)->whereHas('lessons')->orderBy('position', 'asc')->first();
                    if (empty($chapter)) {
                        Toastr::error('No hay lecciones en este curso', 'Error');
                        return redirect()->route('courseDetailsView', $slug);
                    }
                ...
                if (!empty($lesson_id)) {
                    ...
                    Toastr::error('¡No hay lección para este curso!', 'Error');
                    return redirect()->route('courseDetailsView', $slug);
                }


            } else {
                Toastr::error('¡No estás inscrito en este curso!', 'Error');
                return redirect()->route('courseDetailsView', $slug);
            }
